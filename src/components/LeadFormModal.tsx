...
const destinations = [
  ...
  'Kedarnath', 'Badrinath', 'Gangotri', 'Yamnotri', 'Kainchi Dham', 'Kailash Darshan', 'Devi Darshan Himachal',
  'Other'
];

const LeadFormModal = ({ open, setOpen }: { open: boolean, setOpen: (open: boolean) => void }) => {
      ...
      <div className="flex min-h-screen items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <Dialog.Panel className="relative bg-white rounded-3xl shadow-2xl w-full max-w-4xl flex overflow-hidden border border-gray-200">
          ...
          <div className="w-full sm:w-1/2 p-8">
            <button className="absolute top-4 right-4 text-gray-400 hover:text-gray-600" onClick={() => setOpen(false)}>
              <X size={24} />
            </button>

            {showThankYou ? (
              <div className="flex h-full items-center justify-center text-center">
                <h3 className="text-2xl font-bold text-green-600">✅ Thank you! We’ll reach out on WhatsApp.</h3>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <h2 className="text-center text-2xl font-bold text-blue-700">
                  🌍 Book Your First Trip & Get 20% Off!
                </h2>
                <p className="text-center text-gray-600 text-sm mb-6">
                  Unlock exclusive deals, personalized itineraries, and unforgettable travel experiences with Himalayan Ape.
                </p>
